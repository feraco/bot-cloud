...
export const RobotConnectionProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const [connection, setRobot] = useState<RobotConnectionData>({
    ros: undefined,
    transportType: DEFAULT_ROBOT_TRANSPORT,
    capabilities: [],
    online: false,
    connectedRobot: null,
    z4rtcSession: null,
  ...
  const connectToRobotWithInfo = async (robot: Robot, timeoutMs?: number): Promise<void> => {
    try {
      const transportType = getRobotTransportType(robot);
      ...
      if (isZ4rtcRobot(robot)) {
        setConnectionStatus('connecting');
        setLastError(null);

        const bridgeResponse = await z4rtcBridgeService.connect(robot);
        ...
        updateRobot({
          ...
          online: true,
          transportType,
          capabilities: bridgeResponse.capabilities,
          z4rtcSession: bridgeResponse.session,
        });
        setConnectionStatus('connected');
      ...
      updateRobot({
        ...
        z4rtcSession: null,
        capabilities: [],
        transportType: getRobotTransportType(robot),
      });
      setConnectionStatus('error');
      setLastError(
        error instanceof Error ? error.message : 'Unknown connection error'
  ...
  const disconnectRobot = () => {
    ...
    if (connection.transportType === 'z4rtc' && connection.z4rtcSession?.sessionId) {
      z4rtcBridgeService.disconnect(
        connection.z4rtcSession.sessionId,
        connection.z4rtcSession.bridgeUrl
      ).catch((error) => {
        console.error('Failed to disconnect z4rtc session', error);
      });
    }
