...
import useWaypointMissionNavigation from '@/hooks/ros/useWaypointMissionNavigation';
...
interface Waypoint extends WaypointActionFields {
  id: string;
  x: number;
  y: number;
  theta: number;
...
export default function MissionMapView({ 
  ...
  // Sequential waypoint execution lets the UI run robot actions at each reached waypoint.
  const {
    startNavigation: startWaypointNavigation,
    cancelNavigation: cancelWaypointNavigation,
    status: navigationStatus,
    progress: navigationProgress,
    error: navigationError,
    isConnected: nav2Connected,
    isActionServerAvailable,
  } = useWaypointMissionNavigation();
  ...
  const executeWaypointAction = useCallback(
    async (waypoint: Waypoint) => {
      if (!waypoint.action_name) {
        return;
      }

      const actionProvider =
        normalizeWaypointActionProvider(waypoint.action_provider, waypoint.action_name) ??
        DEFAULT_WAYPOINT_ACTION_PROVIDER;

      if (actionProvider === 'z4rtc') {
        throw new Error('z4rtc waypoint actions are saved but mission runtime dispatch is not implemented yet.');
      }

      const robotAction = robotActions[waypoint.action_name as RobotActionTypeName];

      if (!robotAction) {
        throw new Error(`Unsupported waypoint action: ${waypoint.action_name}`);
      }

      const request = parseWaypointActionRequest(waypoint.action_payload as Json | null);

      notificationDispatch({
        type: 'ADD_NOTIFICATION',
        payload: {
          title: 'Waypoint action',
          message: `Running ${getWaypointActionLabel(waypoint.action_name, actionProvider)} at waypoint ${waypoint.order + 1}`,
          type: 'info'
        }
      });
      ...
      await new Promise<void>((resolve, reject) => {
        robotAction.action({
          ...
          input: request?.input,
          callback: () => resolve(),
          failedCallback: (message?: string) => reject(new Error(message || `Failed to execute ${robotAction.label}`)),
        });
      });
    },
    [notificationDispatch, robotActions]
  );

  const handleWaypointReached = useCallback(
    async (waypoint: Waypoint) => {
      if (waypoint.id && activeMissionId) {
        await missionsService.markWaypointReached(waypoint.id, activeMissionId);
      }

      setWaypointReachedStatus((currentStatus) => ({
        ...currentStatus,
        [waypoint.id]: true,
      }));
      ...
      try {
        await executeWaypointAction(waypoint);
      } catch (actionError) {
        const message = actionError instanceof Error ? actionError.message : 'Waypoint action failed';

        if ((waypoint.execution_policy ?? DEFAULT_WAYPOINT_EXECUTION_POLICY) === 'continue_on_error') {
          notificationDispatch({
            type: 'ADD_NOTIFICATION',
            payload: {
              title: 'Waypoint action failed',
              message: `${message}. Continuing to the next waypoint.`,
              type: 'warning'
            }
          });
          return;
    ...
    [activeMissionId, executeWaypointAction, notificationDispatch]
  );
  ...
  const startNavigation = useCallback(async () => {
    ...
    if (!isActionServerAvailable) {
      notificationDispatch({
        ...
        payload: {
          title: 'Navigation Error',
          message: 'Navigation is not available. Ensure the robot is connected and Nav2 is running.',
          type: 'error'
        }
      });
      return;
    ...
    console.log('Starting waypoint navigation with waypoints:', waypoints);
    const success = startWaypointNavigation(waypoints, {
      stopOnFailure: true,
    ...
    if (!success) {
      ...
      notificationDispatch({
        ...
        payload: {
          ...
          message: `Navigating through ${waypoints.length} waypoint${waypoints.length === 1 ? '' : 's'}`,
          type: 'success'
        }
      });
    }
  }, [waypoints, isActionServerAvailable, startWaypointNavigation, handleWaypointReached, navigationError, notificationDispatch]);

  const stopNavigation = useCallback(() => {
    try {
      const success = cancelWaypointNavigation();
      if (success) {
  ...
  }, [cancelWaypointNavigation, notificationDispatch]);
  ...
  useEffect(() => {
    if (isNav2Navigating && nav2CurrentIndex >= 0) {
      ...
      waypoints.forEach((wp, index) => {
        newStatus[wp.id] = index < nav2CurrentIndex;
      });
  ...
  }, [nav2CurrentIndex, isNav2Navigating, waypoints, notificationDispatch]);
  ...
  }, [navigationStatus, navigationError, waypoints, onNavigationComplete, notificationDispatch]);
