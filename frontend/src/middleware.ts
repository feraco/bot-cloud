...
import { isLocalAuthEnabled, LOCAL_AUTH_COOKIE } from '@/utils/local-auth';

export async function middleware(request: NextRequest) {
  const path = request.nextUrl.pathname;

  if (isLocalAuthEnabled()) {
    const hasLocalSession = request.cookies.get(LOCAL_AUTH_COOKIE)?.value === 'true';
    const isAuthRoute = path === '/';
    const isProtectedRoute = path !== '/';

    if (isAuthRoute && hasLocalSession) {
