...
import { isZ4rtcRobot } from '@/types/robot-transport';
...
async function checkSingleRobot(
  ...
  if (isZ4rtcRobot(robot)) {
    try {
      const status = await z4rtcBridgeService.status(robot);
      ...
      console.error(`Error checking z4rtc robot ${robot.name}:`, error);
      return {
        robotId: robot.id,
        status: 'unavailable',
        lastChecked: new Date(),
