...
import {
  DEFAULT_ROBOT_TRANSPORT,
  RobotTransportType,
  getRobotTransportType,
...
export default function FleetPage() {
  ...
  const [robotTransportType, setRobotTransportType] = useState<RobotTransportType>(DEFAULT_ROBOT_TRANSPORT);
  const [robotSerialNumber, setRobotSerialNumber] = useState('');
  const [z4rtcBridgeUrl, setZ4rtcBridgeUrl] = useState('');
  const [z4rtcUseCloud, setZ4rtcUseCloud] = useState(false);
  const [z4rtcG1Mode, setZ4rtcG1Mode] = useState(false);
  ...
  const handleAddRobot = () => {
    ...
    setRobotKey('');
    setRobotType('Go2-R1');
    setRobotTransportType(DEFAULT_ROBOT_TRANSPORT);
    setRobotSerialNumber('');
    setZ4rtcBridgeUrl('');
    setZ4rtcUseCloud(false);
    setZ4rtcG1Mode(false);
    setFormError('');
    setShowAddRobotModal(true);
  };

  const handleEditRobot = (robot: Robot) => {
    const z4rtcConfig = getZ4rtcConnectionConfig(robot);
    ...
    setRobotTransportType(getRobotTransportType(robot));
    setRobotSerialNumber(z4rtcConfig.serialNumber || '');
    setZ4rtcBridgeUrl(z4rtcConfig.bridgeUrl || '');
    setZ4rtcUseCloud(Boolean(z4rtcConfig.useCloud));
    setZ4rtcG1Mode(Boolean(z4rtcConfig.g1Mode));
    setFormError('');
    setShowAddRobotModal(true);
  };
  ...
  const handleSaveRobot = async () => {
    ...
    if (robotTransportType === 'z4rtc' && z4rtcUseCloud && !robotSerialNumber.trim()) {
      setFormError('Serial number is required for z4rtc cloud mode');
      return;
    }
    ...
    const connectionConfig = {
      bridgeUrl: z4rtcBridgeUrl.trim() || undefined,
      serialNumber: robotSerialNumber.trim() || undefined,
      useCloud: z4rtcUseCloud,
      g1Mode: z4rtcG1Mode,
    };
      ...
      if (editingRobot) {
        ...
        const { error } = await supabase
          ...
          .update({
            ...
            type: robotType,
            transport_type: robotTransportType,
            serial_number: robotSerialNumber.trim() || null,
            model_family: robotType.split('-')[0] || null,
                ...
                {robots.map((robot) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center mb-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-500 dark:text-gray-400">
                            {robot.type} • {getRobotTransportType(robot).toUpperCase()}
                          </p>
                        </div>
                      </div>
                      ...
                      {isZ4rtcRobot(robot) && robot.serial_number && (
                        <p className="text-xs text-gray-500 dark:text-gray-400 mb-4 truncate">
      ...
      >
        <div className="space-y-4">
          ...
          <div>
            ...
            <select
              value={robotTransportType}
              onChange={(e) => setRobotTransportType(e.target.value as RobotTransportType)}
              className="w-full p-2 border border-gray-300 dark:border-botbot-darker rounded-md 
                         focus:outline-none focus:ring-2 focus:ring-primary dark:focus:ring-botbot-accent
                         bg-white dark:bg-botbot-dark text-gray-800 dark:text-gray-100"
            >
              <option value="ros">{t('fleet', 'transportRos')}</option>
              <option value="z4rtc">{t('fleet', 'transportZ4rtc')}</option>
              <option value="hybrid">{t('fleet', 'transportHybrid')}</option>
            </select>
          </div>

          {robotTransportType === 'z4rtc' && (
            <>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  {t('fleet', 'serialNumber')}
                </label>
                <InputField
                  value={robotSerialNumber}
                  onChange={(e) => setRobotSerialNumber(e.target.value)}
                  placeholder={t('fleet', 'serialNumberPlaceholder')}
                  className="w-full"
                />
              </div>
              ...
              <div>
                ...
                <InputField
                  value={z4rtcBridgeUrl}
                  onChange={(e) => setZ4rtcBridgeUrl(e.target.value)}
                  placeholder={t('fleet', 'bridgeUrlPlaceholder')}
                  className="w-full"
                />
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                  {t('fleet', 'bridgeUrlHelpText')}
                </p>
              </div>
              ...
              <label className="flex items-center gap-3 text-sm text-gray-700 dark:text-gray-300">
                <input
                  type="checkbox"
                  checked={z4rtcUseCloud}
                  onChange={(e) => setZ4rtcUseCloud(e.target.checked)}
                  className="rounded border-gray-300 text-primary focus:ring-primary"
                />
                <span>{t('fleet', 'useCloud')}</span>
              </label>
              ...
              <label className="flex items-center gap-3 text-sm text-gray-700 dark:text-gray-300">
                <input
                  type="checkbox"
                  checked={z4rtcG1Mode}
                  onChange={(e) => setZ4rtcG1Mode(e.target.checked)}
                  className="rounded border-gray-300 text-primary focus:ring-primary"
                />
                <span>{t('fleet', 'g1Mode')}</span>
              </label>
            </>
          )}
