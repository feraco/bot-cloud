...
export default function Missions() {
  ...
  const saveWaypoints = async (waypoints: any[]) => {
    ...
    try {
      ...
      const waypointsForSave = waypoints.map((wp, index) => ({
        ...
        action_provider: wp.action_name
          ? normalizeWaypointActionProvider(wp.action_provider, wp.action_name) ?? DEFAULT_WAYPOINT_ACTION_PROVIDER
          : null,
        action_name: wp.action_name ?? null,
        action_payload: normalizeWaypointActionPayload(wp.action_payload ?? null),
        execution_policy: wp.action_name
          ? normalizeWaypointExecutionPolicy(wp.execution_policy) ?? DEFAULT_WAYPOINT_EXECUTION_POLICY
          : null,
      }));
  ...
  const transformedWaypoints = useMemo(() => {
    ...
    return selectedMission.waypoints.map((wp: any) => ({
      ...
      action_provider: normalizeWaypointActionProvider(wp.action_provider, wp.action_name),
      action_name: wp.action_name ?? null,
      action_payload: wp.action_payload ?? null,
      execution_policy: normalizeWaypointExecutionPolicy(wp.execution_policy),
    }));
