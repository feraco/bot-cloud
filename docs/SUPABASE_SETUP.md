...
-- Optional transport-aware fields for direct WebRTC integrations such as z4rtc
...
CREATE TABLE IF NOT EXISTS public.waypoints (
    ...
    action_provider TEXT DEFAULT 'ros',
    action_name TEXT,
    action_payload JSONB,
    execution_policy TEXT DEFAULT 'wait_for_success',
...
ALTER TABLE public.waypoints
    ADD COLUMN IF NOT EXISTS action_provider TEXT DEFAULT 'ros',
    ADD COLUMN IF NOT EXISTS action_name TEXT,
    ADD COLUMN IF NOT EXISTS action_payload JSONB,
    ADD COLUMN IF NOT EXISTS execution_policy TEXT DEFAULT 'wait_for_success';
...
-- =============================================
-- Table: mission_maps
-- Stores cloud-hosted planning map previews and metadata
-- =============================================
...
### Cloud Planner Storage

If you want to host the new planner route at `/planner`, also create a private storage bucket named `mission-maps` and add policies scoped to the current user's storage folder. The complete SQL is in `docs/sql/cloud_planner.sql`.

Recommended usage:

- Upload a browser-renderable map preview such as `.png`, `.jpg`, `.webp`, `.svg`, or `.bmp`
- Optionally import the matching ROS occupancy `.yaml` file to preload resolution and origin metadata in the planner
- Keep each file under a user folder such as `mission-maps/<user-id>/<filename>`
